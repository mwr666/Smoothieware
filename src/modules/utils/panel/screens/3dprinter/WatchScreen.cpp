        ...
        case 0:
        {
			...
			if(tm.size() > 0) {
                ...
                for (size_t i = 0; i < 2; ++i) {
					...
					if(temp.designator.front() == 'P' || temp.designator.front() == 'C') {
						if(temp.designator.front() == 'P'){
							off += THEPANEL->lcd->printf("Board:%03d ", t);
						}
						if(temp.designator.front() == 'C') {
							off += THEPANEL->lcd->printf("Drivers:%03d ", t);
						}
					}else {
...
void WatchScreen::draw_graphics()
{
    ...
    THEPANEL->lcd->drawVLine(62, 11, 41); //43
    ...
    if (this->has_fan) {
        y = 11 + (row * (icon_height+1));
        THEPANEL->lcd->bltGlyph(x+35, y, icon_width, icon_height, fan_icon);
        THEPANEL->lcd->setCursorPX(x+35 + icon_width, y);
    ...
    if(tm.size() > 0) {
        for (size_t i = 0; i < tm.size(); i++) {
            ...
            if (temp.designator.front() == 'T') {
                ...
                THEPANEL->lcd->printf("%d/%d\xf8", t, tt);
                row++;
            } else if (temp.designator.front() == 'P') {  // Board temperature
                THEPANEL->lcd->bltGlyph(x, y, icon_width, icon_height, mcu_icon);
                THEPANEL->lcd->printf("%d\xf8", t);
                //row++;
            } else if (temp.designator.front() == 'C') {  // Drivers temperature
                THEPANEL->lcd->bltGlyph(x+30, y, icon_width, icon_height, mot_icon);
				THEPANEL->lcd->setCursorPX(x+30 + icon_width +1, y);
                THEPANEL->lcd->printf("%d\xf8", t);
                row++;
            }
        }
